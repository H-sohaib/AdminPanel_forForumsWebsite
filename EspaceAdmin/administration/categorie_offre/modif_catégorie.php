...
$sql = $cnx->prepare("SELECT * from type_offre where id_type_offre='$id_type_offre'");
$sql->execute();
...
if (isset($_POST['submit'])) {
  if (empty($nom)) {
    ...
    $query1 = $cnx->prepare("UPDATE type_offre set nom_type='$nom_type' where id_type_offre='$id_type_offre'");
    if ($query1->execute()) {
      ...
      header("Location:modif_catégorie.php?id_type_offre=$data[id_type_offre]");
      exit;
    }
  }
}
...
<?php
  ...
  ?>
  <title>Modifier cette catégorie</title>
</head>
          ...
          <div class="max-w-xl mx-auto">
            <div class="flex items-center space-x-5">
              ...
              <div class="block pl-2 font-semibold text-xl self-start text-gray-700">
                <h2 class="leading-relaxed">Modifier cette catégorie  <?= $data['nom_type'] ?></h2>
                <p class="text-sm text-gray-500 font-normal leading-relaxed">Completer le formulaire ci-dessous pour modifier cette catégorie </p>
              </div>
            </div>
            <div class="divide-y divide-gray-200">
              <div class="py-8 text-base leading-6 space-y-4 text-gray-700 sm:text-lg sm:leading-7">
                ...
                <div class="flex flex-col">
                  ...
                  <input value="<?= $data['nom_type'] ?>" name="nom" type="text" class="px-4 py-2 border  w-full sm:text-sm border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500  text-gray-600" placeholder="Entrer le nom de la catégorie" required>
                </div>
